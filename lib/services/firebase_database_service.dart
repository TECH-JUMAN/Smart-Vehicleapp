...
class FirebaseDatabaseService {
  ...
  Stream<SensorDataModel?> getLatestSensorData(String vehicleId) {
    return _firestore
        ...
        .asyncMap((snapshot) async {
          ...
          final model = SensorDataModel.fromMap(data);

          // Calculate new status
          final newStatus = model.calculateStatus();

          // If status changed, update Firestore (and trigger notification)
          if (newStatus != model.status) {
            await doc.reference.update({'status': newStatus});
            print("⚠️ Status updated to: $newStatus");
          }
  ...
  Stream<List<NotificationModel>> getNotifications(String userId) {
    return _firestore
        ...
        .map((snapshot) {
          print(
            '📬 Notifications snapshot: ${snapshot.docs.length} documents for userId: $userId',
          );
  ...
  Future<void> addNotification(NotificationModel notification) async {
    try {
      print(
        '📤 Adding notification: ${notification.id} for userId: ${notification.userId}',
      );
